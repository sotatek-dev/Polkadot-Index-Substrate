...
use pallet_committee::CommitteeOrigin;
...
frame_support::construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
        ...
        GeneralCouncil: pallet_collective::<Instance1>,
        OrmlOracle: orml_oracle::<Instance1>,
        OracleOperatorMembership: pallet_membership::<Instance5>,
	}
);
...
impl pallet_committee::Config for Test {
	...
	type ApprovedByCommitteeOrigin = GovernanceOrigin<AccountId, Test>;
	type Event = Event;
	type WeightInfo = ();
}

impl pallet_membership::Config<OperatorMembershipInstancePINT> for Test {
	type Event = Event;
	type AddOrigin = CommitteeOrigin<Test>;
	type RemoveOrigin = CommitteeOrigin<Test>;
	type SwapOrigin = CommitteeOrigin<Test>;
	type ResetOrigin = CommitteeOrigin<Test>;
	type PrimeOrigin = CommitteeOrigin<Test>;
...
parameter_types! {
	pub const GeneralCouncilMotionDuration: BlockNumber = 7 * 24 * 60 * (60_000 /6000);
...
impl pallet_collective::Config<GeneralCouncilInstance> for Test {
	type Origin = Origin;
	type Proposal = Call;
	type Event = Event;
