...
fn price_pair_should_be_available() {
	new_test_ext().execute_with(|| {
        ...
        assert_ok!(OrmlOracle::feed_values(Origin::signed(ADMIN_ACCOUNT_ID), vec![(ASSET_X_ID, base_submission)]));
        ...
        assert_ok!(OrmlOracle::feed_values(Origin::signed(ADMIN_ACCOUNT_ID), vec![(ASSET_Y_ID, quote_submission)]));
